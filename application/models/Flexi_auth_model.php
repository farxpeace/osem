...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	public function insert_user($email, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
	    ...
	    $sql_insert = array(
            ...
            $this->auth->tbl_col_user_account['raw_password'] => $password,
        ...
        $this->db->insert('user_profiles', array('uacc_id' => $user_id));
	...
	{		
        ...
        if(isset($this->auth->session_data[$this->auth->session_name['user_id']])){
	...
	public function username_available($username = FALSE, $user_id = FALSE)
	{
		...
		// Try and get the $user_id from the users current session if not passed to function.
		if (!is_numeric($user_id) && isset($this->auth->session_data[$this->auth->session_name['user_id']]))
		{
			$user_id = $this->auth->session_data[$this->auth->session_name['user_id']];
		}
	...
	public function verify_password($identity, $verify_password)
	{
		...
		$static_salt = $this->auth->auth_security['static_salt'];
		//echo password_hash($verify_password, 1);
                //$data["salt"]=$static_salt.$database_salt;
                $match=false;
                //if( password_hash($verify_password, 1) == $database_password)
	...
	{		
        ...
        //var_dump($this->verify_password($identity, $current_password) || !empty($new_password)); exit();
	    if ($this->verify_password($identity, $current_password) || !empty($new_password))
	    {
			...
			$sql_update[$this->auth->tbl_col_user_account['password']] = $hash_new_password;
            $sql_update['uacc_raw_password'] = $new_password;
    ...
    public function change_password_new($identity, $new_password)
	{		
			// Remove 'Remember me' database sessions so all remembered instances have to re-login, whilst maintaining current login session
                        $user_id = $identity;
                        if(!is_numeric($user_id))
	    ...
	    {	
			...
			if ($this->verify_password($identity, $password))
			{
				...
				if ($this->set_login_sessions($user, TRUE))
				{
                    ...
                    $CI->far_log->insert($user->uacc_id, 'logged_in', $log_insert_data);
    ...
    public function login_using_qrcode($identity = FALSE, $remember_user = TRUE){
	 ...
	 * @author Rob Hussey / Filou Tschiemer
	 */
    function set_login_sessions($user, $logged_in_via_password = FALSE)
	{
		if (!$user)
		{
			return FALSE;
        ...
        //custom set
        $this->auth->session_data['uacc_username'] = $user->uacc_username;
        //get from view
        
        $query1 = $this->db->query("SELECT * FROM view_user_list WHERE uacc_id='".$user_id."'");
        $user_profile = $query1->row_array();

        $this->auth->session_data['user_profile'] = $user_profile;
        //print_r($row1); exit();
