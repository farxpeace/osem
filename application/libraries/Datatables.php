...
class Datatables
{
        ...
        if(isset($sSearch) && $sSearch != '')
            for($i = 0; $i < count($mColArray); $i++)
                if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
                    if($this->check_cType()){
                        if(strlen($this->select[$mColArray[$i]['data']]) > 0){
    ...
    {
        if($this->query_type == 'manual_query'){
            ...
            if($iLength != '' && $iLength != '-1'){
                if($iStart){
                    $limit_statement = "LIMIT ".$iLength.", ".($iStart? $iStart : 0);
                }else{
                    $limit_statement = "LIMIT ".$iLength;
                }


            }
            //echo $this->query_statement." ".$limit_statement;
            return $this->ci->db->query($this->query_statement." ".$limit_statement);
        }else{
    ...
    private function get_total_results($filtering = FALSE)
    {
        ...
        if($this->query_type == 'manual_query'){
            $query = $this->ci->db->query($this->query_statement, NULL, NULL, FALSE);
        }else{
