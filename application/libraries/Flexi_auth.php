...
class Flexi_auth extends Flexi_auth_lite
{
    ...
    {
        //echo '<pre>'; var_dump($this->CI->flexi_auth_model->login($identity, $password, $remember_user)); echo '</pre>'; exit();
        if ($this->CI->flexi_auth_model->login($identity, $password, $remember_user))
        {
            //check is profile created?
            //If not created, auto insert
            $user_account = $this->CI->flexi_auth->get_user_by_identity_row_array();
            $user_profile = $this->CI->far_users->get_profile('uacc_id', $user_account['uacc_id']);
            if(count($user_profile) == 0){
                $data = array(
                    'uacc_id' => $user_account['uacc_id']
                );
                $this->CI->far_users->insert_profile($data);
            }
     ...
     * @author Rob Hussey
     */
    public function insert_user($email, $username, $password, $user_data, $group_id = FALSE, $activate = FALSE)
    {
