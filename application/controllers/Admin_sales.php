...
class Admin_sales extends MY_Controller
{
    ...
    function sales_report()
    {
        ...
        $this->data['list_sales'] = $this->far_sales->list_sales_by_daterange($filter_date_start, $filter_date_end);



        $this->load->view('admin_sales/admin/sales_report', $this->data);
    }
    ...
    function ajax_graph_monthly_sale(){
        $this->far_auth->allowed_group('3', $this->user['ugrp_id']);
        $postdata = $this->input->post('postdata');

        $list_sale = [];
        $query = $this->db->query("SELECT DATE(STR_TO_DATE(create_dttm, '%Y-%m-%d')) AS sale_date,SUM(grand_total) AS total_pay FROM order_detail WHERE MONTH(create_dttm)='".$postdata['filter_month']."' AND YEAR(create_dttm)='".$postdata['filter_year']."' GROUP BY DATE(STR_TO_DATE(create_dttm, '%Y-%m-%d'))");
        if($query->num_rows() > 0){
            $list_sale = $query->result_array();
            foreach($list_sale as $a => $b){
                $list_sale[$a]['nicedate'] = date("d", strtotime($b['sale_date']));
            }
        }
        $this->data['list_sale'] = $list_sale;
        $this->load->view('admin_sales/admin/ajax_graph_monthly_sale', $this->data);
    }
}