...
class Customer extends MY_Controller
{
    ...
    function ajax_add_new_customer(){
        ...
        if($this->far_customer->is_customer_exists_by_mobile_number($mobile_number)){
            $error['mobile_number'] = "Customer already exists. Please choose customer from dropdown list";
        }else{
            if(strlen($postdata['fullname'] ?? "") < 3){
                $error['fullname'] = "Fullname must be more than 3 characters";
            }
        }

        if(strlen($mobile_number) < 8){
            $error['mobile_number'] = "Mobile Number must be more than 7 digits";
        }
        ...
        if(count($error) == 0){
            ...
            $output['select2_text'] = strtoupper($postdata['fullname'])." (".$mobile_number.")";
            $output['select2_id'] = $customer_id;
